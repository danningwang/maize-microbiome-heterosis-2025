...
bac.rs = read.table("./16S/heterosis/bac.rs.heterosis.sig.res.txt", sep = "\t", header = T)
bac.root = read.table("./16S/heterosis/bac.root.heterosis.significant.res.txt", sep = "\t", header = T)
...
p = ggplot(fungi.df, aes(x=factor(col), y=factor(row), color=Phylum)) +
  ...
  scale_x_discrete(breaks=1:length(unique(fungi.df$Family)), 
                   labels=sort(unique(fungi.df$Family)), 
                   position='top', guide = guide_axis(angle = 90)) +   # set the labels on the X axis
  scale_y_discrete(breaks=1:length(unique(fungi.df$Paternal)), 
                   labels=levels(fungi.df$Paternal)) +                 # set the labels on the Y axis
  theme_bw() +
  theme(axis.line = element_blank(),            # disable axis lines
        axis.title = element_blank(),           # disable axis titles
        panel.border = element_blank(),         # disable panel border
...
# combine the bacteria and fungi data frame
